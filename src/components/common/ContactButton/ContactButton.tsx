...
const icons: Record<string, ReactNode> = {
  ...
  email: (
    <svg viewBox="0 0 24 24" className={styles.icon} aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function ContactButton() {
  ...
  return (
    <div className={`${styles.container} ${inContactSection ? styles.hidden : ""}`}>
      {items.map((item) => (
        <a
          key={item.type}
          href={item.href}
